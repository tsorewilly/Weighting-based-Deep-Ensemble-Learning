...
mlpclf = MLPClassifier(hidden_layer_sizes=(128, 64, 32),  activation="relu", random_state=1)
mlpclf.fit(xTrainRes, yTrainRes)
y_testMLP = mlpclf.predict(xTestRes) - 1
#y_predClf = mlpclf.predict(X_test).argmax(axis=1)
figPlot(y_actl, y_testMLP, saveAs='Output(MLP).png', title='Multi-layer Perceptron')
...
scores = evaluate_model(mlpclf, xTrainRes, yTrainRes)